#include "tnm046.h"  // To be able to use OpenGL extensions below
...
void soupCreateBox(triangleSoup *soup, float xsize, float ysize, float zsize) 
{
	...
	soup->nverts = 36;
	soup->ntris = 12;
	soup->vertexarray = (float*)malloc(sizeof(float)*8*soup->nverts);
	soup->indexarray = (GLuint*)malloc(sizeof(GLuint)*3*soup->ntris);
	
	GLfloat cubeVertexDataArray[] =
	{
		-1.0f, -1.0f, -1.0f, //0		
		1.0f, 1.0f, -1.0f, //2
		1.0f, -1.0f, -1.0f, //1
	...
	for (int i = 0; i < 36; i++)
	{
		soup->vertexarray[i*stride] = xsize * cubeVertexDataArray[i*vertStride];
		soup->vertexarray[(i*stride)+1] = ysize * cubeVertexDataArray[(i*vertStride)+1];
		soup->vertexarray[(i*stride)+2] = zsize * cubeVertexDataArray[(i*vertStride)+2];
		soup->vertexarray[(i*stride)+3] = cubeVertexDataArray[(i*vertStride)];
		soup->vertexarray[(i*stride)+4] = cubeVertexDataArray[(i*vertStride)+1];
		soup->vertexarray[(i*stride)+5] = cubeVertexDataArray[(i*vertStride)+2];
		soup->vertexarray[(i*stride)+6] = cubeUVDataArray[(i*uvStride)];
		soup->vertexarray[(i*stride)+7] = cubeUVDataArray[(i*uvStride+1)];
	}
...
void soupCreateSphere(triangleSoup *soup, float radius, int segments) {
	...
	soup->indexarray = (GLuint*)malloc(soup->ntris * 3 * sizeof(GLuint));
	...
	soup->vertexarray[1] = 0.0f;
	soup->vertexarray[2] = radius;
	...
	soup->vertexarray[base+1] = 0.0f;
	soup->vertexarray[base+2] = -radius;
	soup->vertexarray[base+3] = 0.0f;
	soup->vertexarray[base+4] = 0.0f;
	soup->vertexarray[base+5] = -1.0f;
