...
void loadExtensions() {
    ...
    else
    {
        ...
        if( !glCreateProgram || !glDeleteProgram || !glUseProgram ||
            ...
            !glUniform1fv || !glUniform1f || !glUniformMatrix4fv )
        {
...
GLuint createShader(char *vertexshaderfile, char *fragmentshaderfile) {
    ...
    if(vertexCompiled  == GL_FALSE)
  	{
		char errorType[1024];
		sprintf(errorType,"file: %s, Vertex shader compile error\n",vertexshaderfile);
        glGetShaderInfoLog(vertexShader, sizeof(str), NULL, str);
        printError(errorType, str);
  	}
    ...
    if(fragmentCompiled == GL_FALSE)
   	{
		char errorType[1024];
		sprintf(errorType,"file: %s, Fragment shader compile error\n",vertexshaderfile);
